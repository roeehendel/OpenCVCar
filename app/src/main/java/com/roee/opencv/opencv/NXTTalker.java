...
public class NXTTalker {
    ...
    private synchronized void setState(int state) {
        mState = state;
        if (mHandler != null) {
            mHandler.obtainMessage(DrivingActivity.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
        } else {
            //XXX
        }
    }
    ...
    private void toast(String text) {
        if (mHandler != null) {
            Message msg = mHandler.obtainMessage(DrivingActivity.MESSAGE_TOAST);
            Bundle bundle = new Bundle();
            bundle.putString(DrivingActivity.TOAST, text);
            msg.setData(bundle);
